...
#    model = ALS.train(ratings, rank, numIter) 
    # Save and load model
#    model.save(sc, "target/tmp/rank100num30_1")
    model = MatrixFactorizationModel.load(sc, "target/tmp/rank100num30")
...
#        # rank120, numIter30: MSE??? "target/tmp/rank120num30"
...
#            .map(lambda l: Rating(int(l[0]), int(l[1]), float(0)))
    test = testRating.map(lambda p: (p[0], p[1]))
#    print test.count()
