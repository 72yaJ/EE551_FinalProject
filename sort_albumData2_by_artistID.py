...
# coding: utf-8
'''
File Name: sort_albumData2_by_artistID.py
Edit Time: 20180421 1906

Content:
    sort the albumData2.txt from small to large by artistID, row1
...
def main():
    ...
    file_in = dataDir + 'albumData2.txt'
    t = strftime('%Y%m%d%H%M', gmtime())
    title = 'artistID_albumID'+t+'.txt'
    output_file = dataDir + title   # sorted track data
    fOut = open(output_file, 'w')
    row_num = 1 # artistID is in row 1
...
#        pdb.set_trace()
        ...
        if lastID != nowID:
            ...
            outStr = str(sort_list[ii][row_num])+'|'+str(sort_list[ii][0])
                # sort_list[ii][0] albumID
                # sort_list[ii][1] artistID
                # sort_list[ii][2:] genreID
