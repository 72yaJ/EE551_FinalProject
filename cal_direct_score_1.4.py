...
# coding: utf-8
'''
File Name: cal_direct_score_1.4.py
Edit Time: 20180510 1420
...
def main():
    ...
    for line in f_in:
        ...
        arr_in = line.strip().split('|')
            ...
            # arr_in[11] GenreMean
            # arr_in[12] trackEstimate by matrix factorization
...
#            pdb.set_trace()
        ...
        if n_norm_all == 0 and arr_in[11] != 'None':
            ge_score = w_ar_mean*float(arr_in[11])
            n_norm_all = n_norm_all+w_ge

        if n_norm_all == 0 and arr_in[8] != 'None':
            score = w_tr_mean*float(arr_in[8])+w_tre*float(arr_in[12])
